...
The published image is public in GitHub Container Registry and runs as a
non-root user. The repository includes a default-hardened Compose file with a
persistent named volume:

```bash
cp rustdesk-api.env.example rustdesk-api.env
chmod 600 rustdesk-api.env
${EDITOR:-vi} rustdesk-api.env
docker compose up -d
```

Replace the placeholder `RUSTDESK_API_CREDENTIAL` in `rustdesk-api.env` with
the `username:bcrypt_hash` value generated by `go run ./cmd/gen-credential`.
Keep the credential value in single quotes: Compose treats single-quoted
values literally, so the `$` characters in a bcrypt hash remain intact.

The default Compose setup:

- pulls `ghcr.io/invokerw/rustdesk-minimal-api:latest`
- binds the API to `127.0.0.1:21114` for an HTTPS reverse proxy
- persists `/data/state.json` in the `rustdesk-api-data` volume
- disables unauthenticated inventory uploads
- uses a read-only root filesystem, a non-root user, no new privileges, and no Linux capabilities
...
For a direct LAN-only deployment, edit `docker-compose.yml` and change the
port mapping to `21114:21114`, then restrict that port at the firewall. Do not
publish plain HTTP to the Internet; put Caddy or Nginx in front of it and use
the HTTPS URL in RustDesk clients.

Upgrade while retaining the address book and signing secret:
...
To remove the container but keep data, use `docker compose down`. Do not add
`--volumes` unless you intentionally want to delete the persisted state.
